	...
	<div id="content">
		<div class="post">
         ...
         <!-- start -->
					...
					<div id="slider">
                    <form>
                      <h1>Lista De Personas Beneficiadas</h1>
                      <p>
                      </p>
		    ...
		    $conteo_query =  $mysqli->query("SELECT COUNT(*) AS conteo FROM personas a,proyectos b,madre c,riesgo d,deplazado e,terri f WHERE a.codigo=b.codigo AND a.id_territorio=f.id_territorio AND a.id_madre=c.id_madre AND a.id_ries=d.id_ries AND a.id_des=e.id_des;");
		    $conteo = "";
		    if($conteo_query){
		    ...
		    $max_num_paginas = intval($conteo/8); //en esto caso 10
			
		  
			
			
		    $segmento = $mysqli->query(" SELECT  a.cedula,a.nombres,a.apellidos,a.dirreccion,a.barrio,a.celular,b.nombre_de_proyecto,a.N_caja,a.fecha_ingreso,a.ced_castatral,a.sisben,a.Observaciones,f.territorio,c.situacion,d.estado,e.estado 
		    	FROM personas a,proyectos b,madre c,riesgo d,deplazado e,terri f WHERE a.codigo=b.codigo 
		    	AND a.id_territorio=f.id_territorio AND a.id_madre=c.id_madre AND a.id_ries=d.id_ries AND a.id_des=e.id_des 
		    	LIMIT ".(($page-1)*8).", 8 ");
		     ...
		     if($segmento){
					      ...
					      echo "</tr>";
			          		...
			          		while($obj2 = $segmento->fetch_object()){
			                   ...
			                   <td>'.$obj2->cedula.'</td>
			                   <td>'.$obj2->nombres.'</td>
			                   <td>'.$obj2->apellidos.'</td>
							   <td>'.$obj2->dirreccion.'</td>
			                   <td>'.$obj2->barrio.'</td>
			                   <td>'.$obj2->celular.'</td>
							   <td>'.$obj2->nombre_de_proyecto.'</td>
							   <td>'.$obj2->N_caja.'</td>
			                   <td>'.$obj2->fecha_ingreso.'</td>
			                   <td>'.$obj2->ced_castatral.'</td>
			                   <td>'.$obj2->sisben.'</td>
			                   <td>'.$obj2->Observaciones.'</td>
			                   <td>'.$obj2->territorio.'</td>
			                   <td>'.$obj2->situacion.'</td>
			                   <td>'.$obj2->estado.'</td>
			                   <td>'.$obj2->estado.'</td>
			             </tr>'; 
              ...
              for($i=0; $i<$max_num_paginas;$i++){
		       echo '<a href="lista_personas_general.php?pg='.($i+1).'">'.($i+1).'</a> | ';
		    }     
                           ?> 
                     ...
                     <table>
			          		<tr>
			          		<form target="_blank" action="pdf_general.php" method="get">
			          		<td> <input type ="image" src="logos/adobe.png" ></td>
			          		</form>
			          		
                            <form  target="_blank" action="excel_general.php" method="get">
